    ...
    public class PublicRepository
    {
        ...
        public string Login(Login login)
        {
            try
            {
                ...
                string hashedPassword = Convert.ToString(com.ExecuteScalar());
                
                if (hashedPassword != "0" && !string.IsNullOrEmpty(hashedPassword))
                {
                    if (login.VerifyPassword(hashedPassword))
                    {
                        return "JobSeeker";
                        }
                        else
                ...
                com.CommandType = CommandType.StoredProcedure;
                com.Parameters.AddWithValue("@Username", login.Username);
                hashedPassword = Convert.ToString(com.ExecuteScalar());
